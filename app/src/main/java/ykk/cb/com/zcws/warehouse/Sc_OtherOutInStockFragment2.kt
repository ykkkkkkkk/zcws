...
import kotlinx.android.synthetic.main.ware_sc_other_out_in_stock_fragment2.*
import okhttp3.*
 ...
 * 作者：ykk
 */
class Sc_OtherOutInStockFragment2 : BaseFragment() {

    private val SEL_DEPT = 10
    private val SEL_CUST = 11
    ...
    private var cust:Customer? = null
    ...
    private class MyHandler(activity: Sc_OtherOutInStockFragment2) : Handler() {
        private val mActivity: WeakReference<Sc_OtherOutInStockFragment2>
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.ware_sc_other_out_in_stock_fragment2, container, false)
    }
    ...
    @OnClick(R.id.tv_deptSel, R.id.tv_custSel, R.id.tv_stockSel, R.id.btn_scan_stockPos, R.id.btn_stockPosSel, R.id.btn_scan, R.id.btn_mtlSel, R.id.btn_save, R.id.btn_clone)
...
//                showForResult(Cust_DialogActivity::class.java, SEL_CUST, null)
    ...
    fun checkSelStockPos() : Boolean {
        when (smqFlag) {
            '1' -> {
                if (stock == null) {
                    Comm.showWarnDialog(mContext, "请选择发货仓库！")
                    return false;
                }
                if (stock!!.fisStockMgr == 0) {
                    Comm.showWarnDialog(mContext, "发货仓库没有启用库位！")
                    return false;
                }
            }
            '2' -> {
                ...
                if (getValues(tv_stockPosName).length == 0 && stock!!.fisStockMgr == 1) {
                    Comm.showWarnDialog(mContext, "请选择发货库位！")
                    return false;
                }
            }
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            ...
            SEL_CUST -> {//查询客户	返回
                if (resultCode == Activity.RESULT_OK) {
                    cust = data!!.getSerializableExtra("obj") as Customer
